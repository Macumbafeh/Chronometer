...
function Chronometer:RogueSetup()
	...
	self:AddTimer(self.SPELL, BS["Cheap Shot"],       4, 1,0,0, { dr=1 })
	...
	self:AddTimer(self.SPELL, BS["Ghostly Strike"],   7, 0,1,1)
	self:AddTimer(self.SPELL, BS["Gouge"],            4, 1,0,0, { d={tn=BS["Improved Gouge"],   tb=0.5}, dr=2 })
	self:AddTimer(self.SPELL, BS["Hemorrhage"],      15, 1,0,0)
	self:AddTimer(self.SPELL, BS["Kick"],             5, 1,0,0, { ea={[BS["Kick - Silenced"]]=1} })
	self:AddTimer(self.SPELL, BS["Kidney Shot"],      1, 1,0,0, { d={rs=1, cp=1}, dr=1 })
	self:AddTimer(self.SPELL, BS["Riposte"],          6, 1,0,0)
	self:AddTimer(self.SPELL, BS["Rupture"],          8, 1,0,0, { d={cp=2} })
	self:AddTimer(self.SPELL, BS["Sap"],             25, 1,0,0, { d={rs=10}, dr=2 })
    ...
    self:AddTimer(self.SPELL, BS["Cloak of Shadows"], 5, 0,1,1)
    self:AddTimer(self.SPELL, BS["Shadowstep"],      10, 0,1,1)

	self:AddTimer(self.EVENT, BS["Kick - Silenced"],    2, 1,0,0, { cr="GREEN" })
	self:AddTimer(self.EVENT, BS["Mace Stun Effect"], 3, 1,0,0, { cr="GREEN", a=1, xn=BS["Mace Specialization"], dr=3 })
