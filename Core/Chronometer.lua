		...
		bar = {
			...
			args = {
				texture = {
					...
					get = function () return Chronometer.db.profile.bartex end,
					set = function (v) Chronometer.db.profile.bartex = v end,
					validate = surface:List(),
				},
				color = {
					name = L["Bar Color"], desc=L["Set the default bar color."], type = "text",
					get = function () return Chronometer.db.profile.barcolor end,
					set = function (v) Chronometer.db.profile.barcolor = v end,
					validate = colors,
				},
				bgcolor = {
					name = L["Background Color"], desc=L["Set the bar background color."], type = "text",
					get = function () return Chronometer.db.profile.bgcolor end,
					set = function (v) Chronometer.db.profile.bgcolor = v end,
					validate = colors,
				},
				bgalpha = {
					name = L["Background Alpha"],desc = L["Alpha value of the bar background."],type = "range",
					get = function () return Chronometer.db.profile.bgalpha end,
					set = function (v) Chronometer.db.profile.bgalpha = v end,
					min = 0.0, max = 1.0,
				},
				textcolor = {
					name = L["Text Color"], desc=L["Set the bar text color."], type = "text",
					get = function () return Chronometer.db.profile.textcolor end,
					set = function (v) Chronometer.db.profile.textcolor = v end,
					validate = colors,
				},
				textsize = {
					name = L["Text Size"], desc = L["Set the bar text size."], type = "range",
					get = function () return Chronometer.db.profile.textsize end,
					set = function (v) Chronometer.db.profile.textsize = v end,
					min = 8, max = 20,
				},
				scale = {
					name = L["Bar Scale"], desc=L["Set the bar scale."], type = "range",
					get = function () return Chronometer.db.profile.barscale end,
					set = function (v) Chronometer.db.profile.barscale = v end,
					min = 0.5, max = 1.5,
				},
				height = {
					name = L["Bar Height"], desc=L["Set the bar height."], type = "range",
					get = function () return Chronometer.db.profile.barheight end,
					set = function (v) Chronometer.db.profile.barheight = v end,
					min = 8, max = 30,
				},
				width = {
					name = L["Bar Width"], desc=L["Set the bar width."], type = "range",
					get = function () return Chronometer.db.profile.barwidth end,
					set = function (v) Chronometer.db.profile.barwidth = v end,
					min = 50, max = 300,
				},
				growth = {
					name = L["Bar Growth"], desc=L["Toggles bar growing up or downwards."], type = "toggle",
					get = function () return Chronometer.db.profile.growup end,
					set = function (v) Chronometer.db.profile.growup = v
					Chronometer:SetCandyBarGroupGrowth("Chronometer", Chronometer.db.profile.growup) end,
				},
				reverse = {
					name = L["Reverse"], desc = L["Toggles if bars are reversed (fill up instead of emptying)."], type = "toggle",
					get = function() return Chronometer.db.profile.reverse end,
					set = function(f) Chronometer.db.profile.reverse = f end,
				},
				text = {
					...
					get = function () return Chronometer.db.profile.text end,
					set = function (v) Chronometer.db.profile.text = v end,
				},
			},
		},
		ghost = {name = L["Ghost"], desc = L["Change the amount of time that ghost bars stay up."], type = "range",
			get = function () return Chronometer.db.profile.ghost end,
			set = function (v) Chronometer.db.profile.ghost = v end,
			min = 0, max = 30},
		kill = {
			name = L["Kill"], desc = L["Toggles whether bars disappear when killing things."], type = "toggle",
			get = function() return Chronometer.db.profile.fadeonkill end,
			set = function(f) Chronometer.db.profile.fadeonkill = f end,},
		fade = {
			name = L["Fade"], desc = L["Toggles whether bars disappear when spells fade."], type = "toggle",
			get = function() return Chronometer.db.profile.fadeonfade end,
			set = function(f) Chronometer.db.profile.fadeonfade = f end,},
		self = {
			name = L["Self"], desc = L["Toggles bars for spell durations on the player."], type = "toggle",
			get = function() return Chronometer.db.profile.selfbars end,
			set = function(f) Chronometer.db.profile.selfbars = f end,
		},
...
function Chronometer:OnInitialize()
	...
	local path = "Interface\\AddOns\\Chronometer\\Textures\\"
...
function Chronometer:OnEnable()
	...
	for k, v in pairs(self.dataSetup) do
		v(self)
	end
...
function Chronometer:RunTest()
	for i = 1, 5 do
		...
		if self.db.profile.bartex then self:SetCandyBarTexture(id, surface:Fetch(self.db.profile.bartex)) end
...
function Chronometer:StartTimer(timer, name, target, rank, durmod)
	if not target or target == "" then target = "none" end
	...
	if timer.x.cleangr then
		for idx, gr in ipairs(timer.x.cleangr) do
			self:CleanGroup(gr, target)
		end
	end
	if timer.x.r then
		for i = 1, 20 do
			if self.bars[i].id then
				if self.bars[i].target == target and self.bars[i].name == timer.x.r then
	...
	if self.db.profile.textcolor then self:SetCandyBarTextColor(id, self.db.profile.textcolor) end
	if self.db.profile.bgcolor then self:SetCandyBarBackgroundColor(id, self.db.profile.bgcolor, self.db.profile.bgalpha) end
	self:SetCandyBarFade(id, self.db.profile.ghost, true)
	self:SetCandyBarCompletion(id, self.StopBar, self, id)
...
function Chronometer:OnClick(id, button)
...
function Chronometer:CreateAnchor(text, cRed, cGreen, cBlue)
	...
	if self.db.profile.barposition.x and self.db.profile.barposition.y then
		f:ClearAllPoints()
		f:SetPoint("CENTER", UIParent, "CENTER", self.db.profile.barposition.x, self.db.profile.barposition.y)
	else
...
function Chronometer:GetRankNumber(rank)
	local r
	for rt in string.gmatch(rank, "%d+") do
		r = tonumber(rt)
	end
	return r
end
...
-- Catch Spellcast                                                            --
--<< ====================================================================== >>--
function Chronometer:UNIT_SPELLCAST_SENT(p, spell, rank, target)
	local r = self:GetRankNumber(rank)
	local timer = self.timers[Chronometer.SPELL][spell]
	if timer then
		table.insert(self.captive, 1, {t=timer, n=spell, u=target, r=r})
	end
end
...
function Chronometer:UNIT_SPELLCAST_SUCCEEDED(p, spell, rank)
	local r = self:GetRankNumber(rank)
	for idx, captive in pairs(self.captive) do
		if captive.n == spell and captive.r == r then
...
function Chronometer:UNIT_SPELLCAST_STOP(unit)
	if unit == "player" then
