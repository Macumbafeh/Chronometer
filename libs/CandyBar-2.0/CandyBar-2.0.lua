...
local vmajor, vminor = "CandyBar-2.0", "$Revision: 28043 $"
...
function CandyBar:Register(name, time, text, icon, c1, c2, ...)
	...
	CandyBar:argCheck(text, 4, "string")
	CandyBar:argCheck(icon, 5, "string", "nil")
	CandyBar:argCheck(c1, 6, "string")
	CandyBar:argCheck(c2, 7, "string", "nil")
	...
	if c1 and c2 then
		CandyBar:SetGradient(name, c1, c2, ...)
	end
	return true
end
...
function CandyBar:SetGradient(name, c1, c2, ...)
	CandyBar:argCheck(name, 2, "string")
	CandyBar:argCheck(c1, 3, "string", "nil")
	CandyBar:argCheck(c2, 4, "string", "nil")
	...
	for i = 1, select('#', ...) do
		...
		local t = new()
		_, t[1], t[2], t[3] = paint:GetRGBPercent(c)
...
function CandyBar:SetColor(name, color, alpha)
	...
	handler.color = ctable
	handler.gradient = nil

	handler.frame.statusbar:SetStatusBarColor(unpack(ctable, 1, 4))
...
function CandyBar:SetBackgroundColor(name, color, alpha)
	...
	handler.bgcolor = ctable

	handler.frame.statusbarbg:SetStatusBarColor(unpack(ctable, 1, 4))
...
function CandyBar:SetTextColor(name, color, alpha)
	CandyBar:argCheck(name, 2, "string")
	CandyBar:argCheck(color, 3, "string")
	CandyBar:argCheck(number, 4, "number", "nil")
	...
	handler.textcolor = ctable
	
	handler.frame.text:SetTextColor(unpack(ctable, 1, 4))
...
function CandyBar:SetTimerTextColor(name, color, alpha)
	...
	handler.timertextcolor = ctable
	
	handler.frame.timertext:SetTextColor(unpack(ctable, 1, 4))
...
function CandyBar:SetTimeFormat(name, func, ...)
	...
	local i = 1
	while true do
		local k = "timeformat" .. i
		if handler[k] == nil then
			break
		end
		handler[k] = nil
		i = i + 1
...
function CandyBar:Update(name)
	...
	if handler.timeformat then
		timetext = handler.timeformat(t, getArgs(handler, "timeformat", 1))
	else
	...
	if handler.gradient then
		local p = handler.elapsed / handler.time
		...
		if gstart and gend then
			...
			for i = 1, 4 do
				...
				tmp[i] = gstart[i]*(1-gp) + gend[i]*(gp)
			end
			handler.frame.statusbar:SetStatusBarColor(unpack(tmp, 1, 4))
		end
	end
end
...
function CandyBar:OnClick()
	...
	local button = arg1
	handler.onclick(this.owner, button, getArgs(handler, "onclick", 1))
	return true
end
